...
- **Routes**: Dashboard is `/` (there is **no** `/dashboard` route — navigating there renders a blank page and any load-gate times out). Others: `/sessions`, `/sessions/:id` (session detail), `/workflow`, `/timeline`, `/agents`, `/hooks`.
...
|---|---|---|
| Dashboard | `.dash-bar-row.dash-link .dash-bar-label` (project/skill names only — scoped to the linked rows so it doesn't also catch the Tools panel's plain `.dash-bar-row` tool names), `.dash-toplist-name` (expensive sessions), `.dash-mcp-list` (MCP tool chips), `.dash-err-excerpt` (latest-errors message text — can contain full filesystem paths incl. username) | KPIs, charts, amounts, tool names (Tools panel), `.dash-err-tool`/`.dash-err-ago` (tool name, time), Friction & Behavior panel |
...
| Hooks | The **0.4.0 "Executions (all sessions)" analytics table** is the intended frame (page is very tall, ~7650px — capture the top viewport at scroll 0, which shows the heading, column headers and top rows). In the Hook column (`table tbody tr td:nth-child(1)`, no dedicated class), blur only the tool-name portion of rows referencing an **internal/custom MCP server** — `custom-docker-http__*`, `ccd_session__*`, `claude_ai_Atlassian__*` and similar private servers — keeping the `Event:` prefix sharp. No JS `<span>` split needed at scroll 0: rows are sorted by fire-count, so the top ~28 rows are all public (core Claude Code tools + the public `chrome-devtools`/`Claude_in_Chrome`/`computer-use`/`maurice` MCPs) and nothing in-frame needs blurring — but apply the blur across the whole table defensively (via a content scan, splitting the cell text at the first `:` and blurring the remainder) in case a frame includes internal rows. No filesystem paths appear in this table. | Event grouping (`PreToolUse:`/`PostToolUse:`/`Stop`/`SessionStart:`/`PermissionRequest:`/`Notification:`/`UserPromptSubmit` prefixes), the Fires / Avg duration / Async / Errors / Sessions analytics columns, public tool & MCP names, the "Executions (all sessions)" heading |
