...
- **Detail** — readable thread (errors highlighted, thoughts collapsible), context window fill curve (drop = compaction), cost/token panel, **MCP** tools called, **subagents** (clickable transcripts), and a "copy `claude --resume <id>`" button. First/last page navigation buttons (⟪ / ⟫). Navigate into/out of **rewind-abandoned thread forks**, with a deep link back to the divergence point. A quick-nav panel jumps straight to any **prompt-cache rewrite** warning in the thread.
