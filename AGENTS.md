...
- `server/` — Express API (port `5174`), run with `tsx` (TypeScript executed directly, **imports use explicit `.ts` extensions**).
  ...
  - `server/peers.ts` — `computePeerGraph(index, registry)`: joins the per-session events into cross-session edges (by `msg_id`, with a body-hash fallback for legacy envelopes). Pure, derived from the index on every request, never persisted. Any ambiguity becomes an `unresolved` entry rather than a guessed edge, and a send aimed at an in-process subagent is dropped **before** the joins — left in, its body hash could be picked up by an unrelated receive.
  - `server/continuity.ts` — `computeContinuity(index, meta)`: cross-**file** continuation. A rewind/resume can copy a conversation into a new transcript (new sessionId, regenerated uuids) and carry on there, leaving the original file truncated — invisible to the intra-file fork analysis. The join keys on the preserved API `requestId` sequence (`SessionMeta.requestIds`): same first requestId = same conversation, strict prefix = proven continuation, anything else = `diverged`. Pure and derived per request, like `computeFacets`.
  - `server/sessionDetail.ts` — `readSessionDetail(...)`, the single assembly point for a session detail (local thread + peer resolution + continuity). `routes/api.ts` and the MCP `get_session` tool both go through it, so the two can never serve different shapes.
